...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          octo_user_profile: this._safeGetItem('octo_user_profile'),
          octo_settings: this._safeGetItem('octo_settings'),
          octo_profile: this._safeGetItem('octo_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('octo_viewed_features'),
          developer_mode: this._safeGetItem('octo_developer_mode'),
        ...
        ui: {
          // Tab configuration
          octo_tab_configuration: this._safeGetItem('octo_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('octo_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('octo_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          octo_read_logs: this._safeGetItem('octo_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('octo_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('octo_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.octo_tab_configuration) {
        try {
          this._safeSetItem('octo_tab_configuration', data.ui.octo_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'octo_acknowledged_debug_issues',
        'octo_acknowledged_connection_issue',
        'error_logs',
        'octo_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('octo_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
