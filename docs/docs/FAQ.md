...
For the best experience with octotask, we recommend using the following models from our 19 supported providers:
...
octotask uses a modular provider architecture. To add a new provider:
...
octotask provides multiple ways to access help and documentation:
...
MCP extends octotask's AI capabilities with external tools:
...
??? question "How do I deploy my octotask projects?"
octotask supports one-click deployment to multiple platforms:
...
octotask provides comprehensive Git and GitHub integration:
...
??? question "What's new in octotask?"
Recent major additions to octotask include:
...
octotask offers templates for popular frameworks and technologies:
...
If you're having trouble with MCP integrations:
    ...
    - **Test server connectivity** outside of octotask
