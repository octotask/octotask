...
/**
 * Dynamically import all icons from the local ./icons folder.
 */
const collectionName = 'octotask';
const customIconCollection = globSync('./icons/*.svg').reduce((acc, iconPath) => {
  const iconName = basename(iconPath).replace(/\.svg$/, '');
  acc[collectionName] ??= {};
...
/**
 * Helper to generate an alpha-transparent palette from a hex color.
 */
function generateAlphaPalette(hex: string): Record<number, string> {
...
const BASE_COLORS = {
  ...
  accent: { 50: '#F8F5FF', 100: '#F0EBFF', 200: '#E1D6FF', 300: '#CEBEFF', 400: '#B69EFF', 500: '#9C7DFF', 600: '#8A5FFF', 700: '#7645E8', 800: '#6234BB', 900: '#502D93', 950: '#2D1959' },
  green: { 50: '#F0FDF4', 100: '#DCFCE7', 200: '#BBF7D0', 300: '#86EFAC', 400: '#4ADE80', 500: '#22C55E', 600: '#16A34A', 700: '#15803D', 800: '#166534', 900: '#14532D', 950: '#052E16' },
  orange: { 50: '#FFFAEB', 100: '#FEEFC7', 200: '#FEDF89', 300: '#FEC84B', 400: '#FDB022', 500: '#F79009', 600: '#DC6803', 700: '#B54708', 800: '#93370D', 900: '#792E0D' },
...
export default defineConfig({
  safelist: Object.keys(customIconCollection[collectionName] || {}).map((icon) => `i-${collectionName}:${icon}`),
  shortcuts: {
      ...
      octotask: {
        elements: {
          ...
          actions: {
            ...
            code: { background: 'var(--octotask-elements-actions-code-background)' },
          },
          artifacts: {
          ...
          messages: {
            ...
            code: { background: 'var(--octotask-elements-messages-code-background)' },
            inlineCode: {
          ...
          prompt: { background: 'var(--octotask-elements-prompt-background)' },
          sidebar: {
